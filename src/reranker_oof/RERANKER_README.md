...
```bash
uv run python -m launchers_overfit_blind_b.s03_assemble_dataset --config configs/blind_v5/dataset.yaml  
```

To disable the test_tracks filtering just put to `false` the follwing in `src/reranker_oof/configs/blind_v5/dataset.yaml`:
```yaml
test_tracks:
  enabled: false
  path: data/talkpl-ai/TalkPlayData-Challenge-Track-Metadata/data/test_tracks-00000-of-00001.parquet
```
If such switch is set to `false`, also the steps s01 and s02 must be re-run *(otherwise you will use a sub-optimal aggregation of the candidate generators)*:
```bash
uv run python -m launchers_overfit_blind_b.s01_tune_rrf --config configs/blind_v5/dataset.yaml
uv run python -m launchers_overfit_blind_b.s02_export_best_rrf --config configs/blind_v5/dataset.yaml
```


**Retrain reranker and make inferece:**
```bash
uv run python -u -m launchers_overfit_blind_b.s06_retrain_submit --config configs/blind_v5/xgb_v5.yaml
```
...
uv run python -u -m launchers_overfit_blind_b.s05_tune_reranker --config configs/blind_v5/xgb_v5.yaml
